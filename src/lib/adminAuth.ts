const STORAGE_KEY = 'rof-admin-session-token'

/** Lambda Function URL origin only (no trailing slash). Same in dev and production — set via VITE_ADMIN_AUTH_URL. */
...
export async function adminLogin(password: string): Promise<{ ok: true } | { ok: false; message: string }> {
